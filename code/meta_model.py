...
# CV stacking : train_predicted.txt
y_pheno=pd.read_csv("y_tr_output_full", sep="\n", header=None)

rrBLUP_pred=pd.read_csv("rrBLUP_yHat_train_stacking_predicted.txt", sep="\n",header=None) 
BA_pred=pd.read_csv("BayesA_yHat_train_stacking_predicted.txt", sep="\n",header=None) 
BB_pred=pd.read_csv("BayesB_yHat_train_stacking_predicted.txt", sep="\n",header=None) 
BC_pred=pd.read_csv("BayesC_yHat_train_stacking_predicted.txt", sep="\n",header=None) 
BL_pred=pd.read_csv("BL_yHat_train_stacking_predicted.txt", sep="\n",header=None) 
BRR_pred=pd.read_csv("BRR_yHat_train_stacking_predicted.txt", sep="\n",header=None) 
...
BA_test_pred=pd.read_csv("BayesA_yHat_test_stacking_predicted.txt", sep="\n",header=None) 
BB_test_pred=pd.read_csv("BayesB_yHat_test_stacking_predicted.txt", sep="\n",header=None) 
BC_test_pred=pd.read_csv("BayesC_yHat_test_stacking_predicted.txt", sep="\n",header=None) 
BL_test_pred=pd.read_csv("BL_yHat_test_stacking_predicted.txt", sep="\n",header=None) 
BRR_test_pred=pd.read_csv("BRR_yHat_test_stacking_predicted.txt", sep="\n",header=None) 
...
for i in Epoch :
    ...
    np.savetxt(f"Stacking_predicted_test.txt",test_pred, fmt="%.8f")
    train_pred=linear_model.predict(Stack_final_X_train)
    np.savetxt(f"Stacking_predicted_train.txt",train_pred, fmt="%.8f")

    co1,p1=stats.pearsonr(y_test[:,0], test_pred[:,0])
    co1_1,p1_1=stats.pearsonr(y_train_full[:,0], train_pred[:,0])


    meta_cor=f"Stacking_NN_correlation.txt"
    outfile=open(meta_cor,"w")
    outfile.write(f"stacking_test {co1} stacking_train {co1_1}\n")
    outfile.close()
